...
public interface MeasurementRepo extends JpaRepository<Measurement, Long> {
    ...
    @Query(nativeQuery = true, value="SELECT * FROM measurement where control_object_id = :searchId order by datetime desc limit :limit")
    ArrayList<Measurement> getMeasurementByDatetime(@Param("searchId") Long searchId, @Param("limit") Integer limit);

    @Query(nativeQuery = true, value="SELECT * FROM measurement where control_object_id = :searchId and datetime >= :begin and datetime <= :end order by datetime desc")
    ...
    @Query(nativeQuery = true, value="SELECT * FROM measurement where control_object_id = :searchId order by datetime desc")
